...
const goToPage = (page: number) => {
    if (page >= 1 && page <= props.emailLogs.last_page) {
        router.visit(`/clients/${props.client.id}/email-logs?${buildQueryString(page, perPage.value)}`, {
            preserveState: true,
            preserveScroll: true,
        });
    }
...
const changePerPage = (newPerPage: number) => {
    perPage.value = newPerPage;
    router.visit(`/clients/${props.client.id}/email-logs?${buildQueryString(1, newPerPage)}`, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const applyFilters = () => {
    router.visit(`/clients/${props.client.id}/email-logs?${buildQueryString(1, perPage.value)}`, {
        preserveState: false,
...
const resetFilters = () => {
    ...
    router.visit(`/clients/${props.client.id}/email-logs?page=1&per_page=${perPage.value}`, {
        preserveState: false,
        preserveScroll: false,
    });
};
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: props.client.ownerName || 'Client',
        href: clientsShow(props.client.id).url,
    },
    {
        title: 'Email Logs',
        href: '#',
...
<template>
    <Head :title="`Email Logs - ${props.client.ownerName}`" />
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 overflow-hidden rounded-xl p-2">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-xl font-semibold text-foreground">Email Notification Logs</h1>
                            <p class="text-sm text-muted-foreground">{{ props.client.ownerEmail }}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-card border border-border rounded-lg p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-semibold text-foreground">{{ props.emailLogs.total }}</p>
                        </div>
            ...
            <div class="flex items-center justify-between gap-4">
                ...
                <!-- Pagination Controls -->
                <div
                    v-if="props.emailLogs.last_page > 1 || props.emailLogs.total > 10"
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex items-center gap-1">
                            <button
                                ...
                                :disabled="props.emailLogs.current_page === 1"
                            ...
                            <button
                                @click="goToPage(props.emailLogs.current_page + 1)"
                                :disabled="props.emailLogs.current_page === props.emailLogs.last_page"
                                class="p-1 rounded-md hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer transition-colors"
                                type="button"
                                title="Next"
                            >
            ...
            <div class="flex-1 overflow-y-auto bg-card border border-border rounded-lg">
                <div class="overflow-x-auto">
                    <table class="w-full">
                        ...
                        <tbody>
                            <tr v-if="props.emailLogs.data.length === 0">
