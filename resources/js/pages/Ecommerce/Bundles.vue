    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-2 overflow-hidden rounded-xl p-2">
            ...
            <div class="flex flex-col gap-2">
                <div>
                    <h1 class="text-base font-semibold text-foreground">Bundle Offers</h1>
                </div>
                            ...
                            <tr v-if="bundles.length === 0">
                                <td colspan="5" class="px-4 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center gap-2">
                                        <p class="text-sm text-muted-foreground">No bundles found</p>
                                    </div>
                                </td>
                            </tr>
                            <tr
                            ...
                            >
                                <td class="px-4 py-3 align-middle">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <span class="text-foreground font-medium">{{ bundle.name }}</span>
                                            <p class="text-xs text-muted-foreground">{{ bundle.description }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3 align-middle">
                                    <div class="flex flex-col gap-1">
                                        <span class="text-sm text-foreground">{{ bundle.productNames.join(', ') }}</span>
                                        <span class="text-xs text-muted-foreground font-mono">{{ bundle.products.join(', ') }}</span>
                                    </div>
                                </td>
                                <td class="px-4 py-3 align-middle text-right">
                                    <div class="flex flex-col items-end">
                                        <span class="text-sm font-medium text-green-600 dark:text-green-400">{{ bundle.discount }}% OFF</span>
                                        <span class="text-xs text-muted-foreground line-through">{{ formatCurrency(bundle.originalPrice) }}</span>
                                    </div>
                                </td>
                                <td class="px-4 py-3 align-middle text-right">
                                    <div class="flex flex-col items-end">
                                        <span class="text-lg font-bold text-foreground">{{ formatCurrency(bundle.price) }}</span>
                                        <span class="text-xs text-green-600 dark:text-green-400">Save {{ formatCurrency(bundle.savings) }}</span>
                                    </div>
                                </td>
                                <td class="px-4 py-3 align-middle">
                                    <div class="flex items-center gap-2">
                                        <Link :href="`/ecommerce/bundles/${bundle.id}/edit`">
                                            <button
