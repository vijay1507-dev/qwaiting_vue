...
import {
    usage as couponUsage,
    destroy as destroyCoupon,
    store as storeCoupon,
    update as updateCoupon,
} from '@/routes/subscription/coupons';
import {
    destroy as destroyFeature,
    store as storeFeature,
    update as updateFeature,
} from '@/routes/subscription/features';
import {
    destroy as destroyPackage,
    store as storePackage,
    update as updatePackage,
} from '@/routes/subscription/packages';
import {
    get as getPackageConfigurationRoute,
    update as updatePackageConfigurationRoute,
} from '@/routes/subscription/packages/configuration';
import {
    destroy as destroyPricing,
    get as getPackagePricing,
    store as storePricing,
    update as updatePricing,
} from '@/routes/subscription/packages/pricing';
...
const hasPermission = (permission: string) => {
    ...
    // Super admin check (assuming specific logic or just wildcard)
    if (
        permissions.includes('*') ||
        permissions.includes('subscription_management.*')
    )
        return true;
    ...
    const allTabs = [
        {
            ...
            label: 'Features',
            icon: FileText,
            permission: 'subscription_management.features.read',
        },
        {
            id: 'packages',
            label: 'Packages',
            icon: Package,
            permission: 'subscription_management.packages.read',
        },
        {
            id: 'configuration',
            label: 'Configuration',
            icon: Settings,
            permission: 'subscription_management.configuration.read',
        },
        {
            id: 'pricing',
            label: 'Pricing',
            icon: DollarSign,
            permission: 'subscription_management.pricing.read',
        },
        {
            id: 'coupons',
            label: 'Coupons',
            icon: Ticket,
            permission: 'subscription_management.coupons.read',
        },
        {
            id: 'preview',
            label: 'Preview',
            icon: Eye,
            permission: 'subscription_management.preview.read',
        },
    ];
...
// Update active tab if default is not available
watch(
    tabs,
    (newTabs) => {
        if (
...
watch(
    [() => activeTab.value, () => packages.value],
    ([newTab, newPackages]) => {
        if (
            newTab === 'configuration' &&
...
const handleUpdateFeature = () => {
    if (
        ...
        !editFeature.value.name ||
        !editFeature.value.dataType
    ) {
        return;
    }
    ...
    const payload = {
        ...
        key:
            editFeature.value.key ||
            editFeature.value.name.toLowerCase().replace(/\s+/g, '_'),
...
const handleSubmitPackage = () => {
    ...
    router.post(
        storePackage().url,
        {
            ...
            code:
                newPackage.value.code ||
                newPackage.value.name.toLowerCase().replace(/\s+/g, '_'),
            ...
            features_display_limit:
                newPackage.value.featuresDisplayLimit || null,
        },
        {
            preserveScroll: true,
            onSuccess: () => {
...
const handleUpdatePackage = () => {
    ...
    if (
        ...
        !editPackage.value.name ||
        !editPackage.value.status
    ) {
        return;
    }
    ...
    router.put(
        updatePackage(editingPackage.value.id).url,
        {
            ...
            code:
                editPackage.value.code ||
                editPackage.value.name.toLowerCase().replace(/\s+/g, '_'),
            ...
            features_display_limit:
                editPackage.value.featuresDisplayLimit || null,
        },
        {
            preserveScroll: true,
            onSuccess: () => {
...
watch(selectedPackage, async (packageId) => {
    ...
    try {
        const response = await fetch(
            getPackageConfigurationRoute(packageId).url,
        );
...
const handleSaveConfiguration = () => {
    ...
    router.put(
        updatePackageConfigurationRoute(selectedPackage.value).url,
        {
            features: features,
        },
        {
...
const handleCreateFeature = () => {
    ...
    router.post(
        storeFeature().url,
        {
            ...
            key:
                newFeature.value.key ||
                newFeature.value.name.toLowerCase().replace(/\s+/g, '_'),
...
const allBillingCyclesAdded = computed(() => {
    ...
    return (
        existingCycles.includes('monthly') && existingCycles.includes('annual')
    );
});
...
const currentPackageName = computed(() => {
    ...
    const pkg = packages.value.find(
        (p) => p.id === selectedPackageForPricing.value,
    );
    return pkg ? pkg.name : '';
});
...
watch(
    () => selectedPackageForPricing.value,
    (packageId) => {
        if (packageId) {
            const pkg = packages.value.find((p) => p.id === packageId);
...
watch(
    [() => selectedPackageForPricing.value, () => selectedCurrency.value],
    async ([packageId, currency]) => {
...
const handleSubmitPrice = () => {
    // Validate required fields
    if (
        !newPrice.value.billingCycle ||
        newPrice.value.price === null ||
        newPrice.value.price === undefined ||
        !selectedPackageForPricing.value
    ) {
        return;
    }
    ...
    router.post(
        storePricing(selectedPackageForPricing.value).url,
        {
...
const handleUpdatePrice = () => {
    ...
    router.put(
        updatePricing([selectedPackageForPricing.value, editingPrice.value.id])
            .url,
        {
...
const confirmDeletePrice = () => {
    if (priceToDelete.value !== null && selectedPackageForPricing.value) {
        router.delete(
            destroyPricing(selectedPackageForPricing.value, priceToDelete.value)
                .url,
            {
                preserveScroll: true,
                onSuccess: () => {
...
const paginationInfoCoupons = computed(() => {
    ...
    const end = Math.min(
        currentPageCoupons.value * perPageCoupons.value,
...
const handleEditPackageSelection = (
...
const handleSubmitCoupon = () => {
    ...
    if (
        ...
        !newCoupon.value.durationType ||
        !newCoupon.value.validFrom ||
    ...
    if (
        newCoupon.value.applicablePackages === 'specific' &&
        newCoupon.value.selectedPackages.length === 0
    ) {
        console.error(
            'Validation failed: No packages selected but applicablePackages is "specific"',
            {
    ...
    const payload: any = {
        ...
        currency:
            newCoupon.value.discountType === 'fixed'
                ? newCoupon.value.currency
                : null,
        duration_type: newCoupon.value.durationType,
        number_of_months:
            newCoupon.value.durationType === 'recurring'
                ? newCoupon.value.numberOfMonths
                : null,
        usage_limit:
            newCoupon.value.usageLimit === 0
                ? null
    ...
    if (newCoupon.value.applicablePackages === 'specific') {
        payload.package_ids = newCoupon.value.selectedPackages.map((id) =>
            Number(id),
        );
    }
...
const usageStats = computed(() => {
    ...
    const totalDiscount = usageHistoryData.value.reduce(
        (sum, usage) => sum + usage.discount,
        0,
    );
    const uniqueCustomers = new Set(
        usageHistoryData.value.map((u) => u.customerEmail),
    ).size;
...
const initializeEditCouponDatePickers = () => {
    ...
    setTimeout(() => {
        ...
        if (editValidFromInputRef.value) {
            try {
                editValidFromPicker.value = flatpickr(
                    editValidFromInputRef.value,
                    {
                        dateFormat: 'Y-m-d',
                ...
                if (editCoupon.value.validFrom && editValidFromPicker.value) {
                    editValidFromPicker.value.setDate(
                        editCoupon.value.validFrom,
                        false,
                    );
                }
            } catch (error) {
                console.error(
                    'Error initializing Valid From date picker:',
                    error,
                );
            }
        }
        ...
        if (editValidUntilInputRef.value) {
            try {
                editValidUntilPicker.value = flatpickr(
                    editValidUntilInputRef.value,
                    {
                        dateFormat: 'Y-m-d',
                ...
                if (editCoupon.value.validUntil && editValidUntilPicker.value) {
                    editValidUntilPicker.value.setDate(
                        editCoupon.value.validUntil,
                        false,
                    );
                }
            } catch (error) {
                console.error(
                    'Error initializing Valid Until date picker:',
                    error,
                );
            }
        }
...
const handleEditCoupon = async (coupon: any) => {
    ...
    const currency = isPercentage
        ? 'USD'
        : coupon.discount.match(/[A-Z]{3}/)?.[0] || 'USD';

    // Map coupon type to duration type
    const durationType =
        coupon.type === 'once'
            ? 'once'
            : coupon.type === 'recurring'
              ? 'recurring'
              : coupon.type === 'forever'
                ? 'forever'
                : 'once';
    ...
    const packageIds =
        coupon.packageIds ||
        coupon.package_ids ||
        coupon.packages?.map((p: any) => String(p.id)) ||
        [];
    ...
    const applicablePackagesFromDB =
        coupon.applicablePackages || coupon.applicable_packages;
    const applicablePackages =
        applicablePackagesFromDB ||
    ...
    editCoupon.value = {
        ...
        selectedPackages: Array.isArray(packageIds)
            ? packageIds.map((id: string | number) => String(id))
            : [],
    ...
    const finalApplicablePackages =
        applicablePackagesFromDB || applicablePackages;
...
const handleUpdateCoupon = () => {
    // Validate required fields
    if (
        !editCoupon.value.code ||
        !editCoupon.value.discountType ||
        !editCoupon.value.discountValue ||
        !editCoupon.value.durationType ||
        !editCoupon.value.validFrom ||
        !editCoupon.value.validUntil
    ) {
        return;
    ...
    // Validate that packages are selected when "specific" is chosen
    if (
        editCoupon.value.applicablePackages === 'specific' &&
        editCoupon.value.selectedPackages.length === 0
    ) {
        return;
    }
    ...
    const payload: any = {
        ...
        currency:
            editCoupon.value.discountType === 'fixed'
                ? editCoupon.value.currency
                : null,
        duration_type: editCoupon.value.durationType,
        number_of_months:
            editCoupon.value.durationType === 'recurring'
                ? editCoupon.value.numberOfMonths
                : null,
        usage_limit:
            editCoupon.value.usageLimit === 0
                ? null
                : editCoupon.value.usageLimit,
    ...
    if (editCoupon.value.applicablePackages === 'specific') {
        payload.package_ids = editCoupon.value.selectedPackages.map((id) =>
            Number(id),
        );
    }
...
watch(showEditCouponModal, async (isOpen) => {
    if (isOpen) {
        ...
        const allRadio = document.getElementById(
            'edit-packages-all',
        ) as HTMLInputElement;
        const specificRadio = document.getElementById(
            'edit-packages-specific',
        ) as HTMLInputElement;
        if (allRadio && specificRadio) {
            allRadio.checked = editCoupon.value.applicablePackages === 'all';
            specificRadio.checked =
                editCoupon.value.applicablePackages === 'specific';
        }
...
const fetchPreviewData = async (currency: string) => {
    ...
    try {
        const response = await fetch(
            subscriptionPreview().url + `?currency=${currency}`,
        );
        const data = await response.json();
...
watch(
    [() => activeTab.value, () => displayCurrency.value],
    async ([newTab, newCurrency]) => {
        if (newTab === 'preview') {
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        >
            ...
            <div class="flex-1 overflow-y-auto">
                ...
                >
                    ...
                    <!-- Quick Tips Section -->
                    <div
                        class="rounded-lg border-l-4 border-blue-500 bg-blue-50 p-4 dark:bg-blue-900/10"
                    >
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1">
                                <h3
                                    class="mb-2 text-sm font-semibold text-blue-900 dark:text-blue-300"
                                >
                                    Quick Tips
                                </h3>
                                <ul
                                ...
                                >
                                    <li>
                                        • Feature Key is auto-generated and used
                                        in your system code
                                    </li>
                                    <li>
                                        • Use <strong>Number</strong> type for
                                        countable limits (projects, users,
                                        storage GB)
                                    </li>
                                    <li>
                                        • Use <strong>Boolean</strong> for
                                        on/off features (API access, custom
                                        domain)
                                    </li>
                                    <li>
                                        • Enable <strong>Metered</strong> only
                                        if you track usage for billing purposes
                                    </li>
                                    <li>
                                        • Set to <strong>Inactive</strong> to
                ...
                >
                    ...
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-muted-foreground">
                                Create subscription tiers that customers can
                                purchase (Basic, Pro, Enterprise)
                            </p>
                        </div>
                        <Button
                    ...
                    >
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1">
                                ...
                                >
                                    <li>
                                        • Create 3-4 packages maximum to avoid
                                        overwhelming customers
                                    </li>
                                    <li>
                                        • Common pattern: Free/Basic → Pro →
                                        Enterprise
                                    </li>
                                    <li>
                                        • Enable both Monthly and Annual billing
                                        for flexibility
                                    </li>
                                    <li>
                                        • Use Trial Period to let customers test
                                        premium features (7-30 days typical)
                                    </li>
                                    <li>
                                        • Credit Card Required determines if
                                        payment info is needed during signup
                                    </li>
                                    <li>
                                        • After creating packages, go to
                                        Configuration to set feature limits
                                    </li>
                                </ul>
                            </div>
                        </div>
                    ...
                    >
                        ...
                        >
                            ...
                            <!-- Billing Options -->
                            <div
                                v-if="pkg.monthlyEnabled || pkg.annualEnabled"
                                style="
                                    display: flex;
                                    gap: 8px;
                                    margin-bottom: 16px;
                            ...
                            <div class="flex gap-2 border-t border-border pt-4">
                                <button
                                    ...
                                    class="flex flex-1 cursor-pointer items-center justify-center gap-2 rounded-md p-1.5 transition-colors hover:bg-muted"
                                    title="Edit"
                                    type="button"
                                >
                                    <Edit
                                ...
                                <button
                                    ...
                                    class="flex flex-1 cursor-pointer items-center justify-center gap-2 rounded-md p-1.5 transition-colors hover:bg-red-50 dark:hover:bg-red-900/20"
                                    title="Delete"
                                    type="button"
                                >
                                    <Trash2
                ...
                >
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-muted-foreground">
                            Define feature availability and limits for each
                    ...
                    <div
                        class="rounded-lg border border-yellow-200 bg-yellow-50 p-4 dark:bg-yellow-900/10"
                    >
                        <div class="flex items-start gap-3">
                            <div class="flex-shrink-0">
                                <Settings
                            ...
                            <div class="flex-1">
                                ...
                                <ul
                                    class="space-y-1 text-sm text-gray-800 dark:text-gray-200"
                                >
                                    <li>
                                        • <strong>Limited:</strong> Set specific
                                        number (e.g., 5 projects, 10GB storage,
                                        1000 API calls)
                                    </li>
                                    <li>
                                        • <strong>Unlimited:</strong> No
                                        restrictions - typically used for
                                        premium packages
                                    </li>
                                    <li>
                                        • <strong>Disabled:</strong> Feature not
                    ...
                    <div class="ml-2 flex w-fit flex-col gap-2">
                        ...
                        <div class="relative">
                            <select
                                ...
                                class="w-64 appearance-none rounded-md border border-input bg-background px-3 py-2 pr-8 text-sm focus:ring-2 focus:ring-ring focus:outline-none"
                    ...
                    >
                        ...
                        <p class="text-sm text-muted-foreground">
                            Please select a package from the dropdown above to
                            configure its features.
                        </p>
                    </div>
                    ...
                    <div
                        v-else-if="
                            selectedPackage && configurationData.length > 0
                        "
                        class="rounded-lg border border-border bg-card"
                    >
                        <div class="border-b border-border p-4">
                            <h3 class="text-lg font-semibold text-foreground">
                                Configure: {{ selectedPackageName }}
                            </h3>
                        </div>
                ...
                >
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-muted-foreground">
                            Set pricing for packages across different currencies
                            and billing cycles.
                        </p>
                    </div>
                    ...
                    <div
                        class="rounded-lg border border-green-200 bg-green-50 p-4 dark:bg-green-900/10"
                    >
                        <div class="flex items-start gap-3">
                            <div class="flex-shrink-0">
                                <DollarSign
                                    class="size-5 text-green-700 dark:text-green-400"
                                />
                            </div>
                            <div class="flex-1">
                                <h3
                                    class="mb-2 text-sm font-semibold text-green-900 dark:text-green-300"
                                >
                                    Pricing Strategies
                                </h3>
                                <ul
                                    class="space-y-1 text-sm text-green-800 dark:text-green-200"
                                >
                                    <li>
                                        • Offer
                                        <strong>15-20% discount</strong> on
                                        annual plans to encourage yearly
                                        commitment
                                    </li>
                                    <li>
                                        • Use
                                        <strong>psychological pricing</strong>
                                        ($29 instead of $30, $99 instead of
                                        $100)
                                    </li>
                                    <li>
                                        • Set <strong>$0.00</strong> pricing for
                                        free tier packages
                                    </li>
                                    <li>
                                        • Add separate prices for
                                        <strong>each currency</strong> you want
                                        to support
                                    </li>
                                    <li>
                                        • Each package can have
                                        <strong
                                            >one price per billing cycle per
                                            currency</strong
                                        >
                                        (no duplicates)
                                    </li>
                    ...
                    <div
                        class="mr-2 ml-2 grid grid-cols-1 gap-4 md:grid-cols-2"
                    >
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="relative">
                                <select
                                    ...
                                    class="w-full appearance-none rounded-md border border-input bg-background px-3 py-2 pr-8 text-sm focus:ring-2 focus:ring-ring focus:outline-none"
                                >
                                    <option value="">
                                        -- Choose a package --
                                    </option>
                    ...
                    <div
                        v-else-if="
                            selectedPackageForPricing && selectedCurrency
                        "
                        class="rounded-lg border border-border bg-card"
                    >
                                            ...
                                            >
                                                ...
                                                >
                                                    <p
                                                        class="text-sm text-muted-foreground"
                                                    >
                                                        No pricing set for this
                                                        package and currency.
                                                        Click "Add Price" to get
                                                        started.
                                                    </p>
                                                </div>
                                            </td>
                    ...
                    >
                        <p class="text-sm text-muted-foreground">
                            Select a package and currency to manage pricing.
                        </p>
                    </div>
                </div>
                ...
                >
                    ...
                    <div class="items-center justify-between">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-muted-foreground">
                                See how your subscription packages will appear
                                to customers.
                            </p>
                        </div>
                        <div class="flex items-center gap-2">
                            ...
                            <div class="relative">
                                <select
                                    ...
                                    class="appearance-none rounded-md border border-input bg-background px-3 py-2 pr-8 text-sm focus:ring-2 focus:ring-ring focus:outline-none"
                                >
                                    <option
                                        v-for="currency in currencies"
                                        :key="currency"
                    ...
                    <!-- Preview Section -->
                    <div
                        style="background-color: #e4e4f7"
                        v-else
                        class="rounded-lg border border-border bg-muted/30 p-8"
                    >
                        ...
                        <!-- Empty State -->
                        <div
                            v-if="previewPackages.length === 0"
                            class="py-12 text-center"
                        >
                            <p class="text-muted-foreground">
                                No active packages available for preview.
                            </p>
                        </div>
                        ...
                        >
                            ...
                            >
                                <!-- Most Popular Badge -->
                                <div
                                    v-if="pkg.isMostPopular"
                                    class="absolute -top-3 left-1/2 -translate-x-1/2"
                                >
                                    <span
                                        class="flex items-center gap-1 rounded-full bg-pink-500 px-3 py-1 text-xs font-semibold text-white"
                                    >
                                ...
                                <!-- Plan Name -->
                                <h3
                                    class="mb-2 text-xl font-bold text-foreground"
                                >
                                    {{ pkg.name }}
                                </h3>
                                ...
                                <div class="mb-6">
                                    <div
                                        v-if="
                                            ...
                                            pkg.annualPrice !== null
                                        "
                                        class="mb-1 text-3xl font-bold text-foreground"
                                    >
                                        <span v-if="pkg.monthlyPrice !== null"
                                            >{{ displayCurrency }}
                                            {{
                                        ...
                                        <span v-if="pkg.annualPrice !== null"
                                            >{{ displayCurrency }}
                                            {{
                                                pkg.annualPrice.toFixed(2)
                                            }}</span
                                        >
                                    </div>
                                    <div
                                    ...
                                    <p class="text-xs text-muted-foreground">
                                        ...
                                        >
                                        <span
                                            v-else-if="pkg.annualPrice !== null"
                                            >Per year</span
                                        >
                                ...
                                <div class="mb-6 space-y-2">
                                    ...
                                    >
                                        <Check
                                            ...
                                            class="size-4 flex-shrink-0 text-green-600"
                                        />
                                        <X
                                            v-else
                                ...
                                <div
                                    v-if="pkg.trialDays"
                                    class="mt-4 flex items-center justify-center gap-1 text-xs text-muted-foreground"
                                >
                                    <Gift class="size-3" />
                                    <span
                                        >{{ pkg.trialDays }}-day free
                                        trial</span
                                    >
                                </div>
                            </div>
                    ...
                    >
                        <div class="flex items-start gap-3">
                            <Lightbulb
                                class="mt-0.5 size-5 flex-shrink-0 text-muted-foreground"
                            />
                            <div class="text-sm text-muted-foreground">
                                <span class="font-medium">Preview Tips:</span>
                                This shows how your active packages will appear
                                to customers. The middle package is
                                automatically highlighted as "Most Popular" when
                                you have 3+ packages. Inactive packages are
                                hidden from preview.
                            </div>
                        </div>
                    </div>
                ...
                >
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-muted-foreground">
                                Create discount codes and promotional offers for
                    ...
                    <div
                        class="rounded-lg border border-blue-200 bg-blue-50/50 p-6"
                    >
                        <div class="flex items-start gap-3">
                            <Ticket
                                class="mt-0.5 size-5 flex-shrink-0 text-blue-600"
                            />
                            <div class="flex-1">
                                <h3
                                ...
                                <div
                                    class="grid grid-cols-1 gap-4 text-sm text-muted-foreground md:grid-cols-2"
                                >
                                    <div>
                                        <span
                                            class="font-medium text-foreground"
                                            >Percentage:</span
                                        >
                                        Discount by % (e.g., 20% off = enter 20)
                                    </div>
                                    <div>
                                        <span
                                            class="font-medium text-foreground"
                                            >Fixed Amount:</span
                                        >
                                        Discount by specific amount (e.g., $50
                                        off)
                                    </div>
                                    <div>
                                        <span
                                            class="font-medium text-foreground"
                                            >Recurring vs Once:</span
                                        >
                                        Apply discount to multiple billing
                                        cycles or just first payment
                                    </div>
                                    <div>
                                        <span
                                            class="font-medium text-foreground"
                                            >Usage Limits:</span
                                        >
                                        Set maximum redemptions or unlimited
                                        usage
                                    </div>
                                    <div class="md:col-span-2">
                                        ...
                                        >
                                        Set start and end dates for the coupon
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            <div class="flex items-center gap-2">
                                ...
                                <select
                                    ...
                                    class="rounded-md border border-input bg-background px-3 py-1.5 text-sm focus:ring-2 focus:ring-ring focus:outline-none"
                                >
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <Button
                                ...
                                size="sm"
                                :disabled="
                                    currentPageCoupons === totalPagesCoupons
                                "
                                @click="goToPageCoupons(currentPageCoupons + 1)"
                                class="cursor-pointer"
                            >
        ...
        <Dialog
            ...
            @update:open="showAddFeatureModal = $event"
        >
            <DialogContent
                ...
                <div class="space-y-4 py-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea
                            ...
                            rows="4"
                            class="w-full resize-none rounded-md border border-input bg-background px-3 py-2 text-sm focus:ring-2 focus:ring-ring focus:outline-none"
                        ></textarea>
                    </div>
        ...
        <Dialog
            ...
            @update:open="showEditFeatureModal = $event"
        >
            <DialogContent
                class="max-h-[90vh] overflow-y-auto sm:max-w-[600px]"
            >
        ...
        <Dialog
            ...
            @update:open="showCreatePackageModal = $event"
        >
            <DialogContent
                class="max-h-[90vh] overflow-y-auto sm:max-w-[600px]"
            >
                ...
                <div class="space-y-4 py-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <p class="text-xs text-muted-foreground">
                            Select at least one billing type. You can enable
                    ...
                    <div class="space-y-2">
                        ...
                        />
                        <p class="text-xs text-muted-foreground">
                            Lower numbers appear first on the frontend pricing
                            page. Default is 0.
                        </p>
                    </div>
                    ...
                    <div class="space-y-2">
                        <Label
                            for="package-features-display-limit"
                            class="text-sm font-medium text-foreground"
                        >
                            Features Display Limit
                        </Label>
                        ...
                        />
                        <p class="text-xs text-muted-foreground">
                            Maximum number of features to display in pricing
                            cards. Leave empty to show all features.
                        </p>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            >
                                <option
                                    v-for="status in packageStatusOptions"
                                    :key="status"
                                    :value="status"
                                >
                                    {{
                ...
                <DialogFooter>
                    ...
                    <Button
                        ...
                        :disabled="
                            !newPackage.name ||
                            !newPackage.code ||
                            (!newPackage.monthlyEnabled &&
                                !newPackage.annualEnabled)
                        "
                    >
                        Create Package
                    </Button>
        ...
        <Dialog
            ...
            @update:open="showEditPackageModal = $event"
        >
            <DialogContent
                class="max-h-[90vh] overflow-y-auto sm:max-w-[600px]"
            >
                ...
                <div class="space-y-4 py-4">
                    ...
                    <div class="space-y-2">
                        <Label
                            for="edit-package-display-sequence"
                            class="text-sm font-medium text-foreground"
                        >
                            Display Sequence
                        </Label>
                    ...
                    <div class="space-y-2">
                        <Label
                            for="edit-package-features-display-limit"
                            class="text-sm font-medium text-foreground"
                        >
                            Features Display Limit
                        </Label>
                ...
                <DialogFooter>
                    ...
                    <Button
                        ...
                        :disabled="
                            !editPackage.name ||
                            (!editPackage.monthlyEnabled &&
                                !editPackage.annualEnabled)
                        "
                    >
                        Update Package
                    </Button>
        ...
        <Dialog
            ...
            @update:open="showAddPriceModal = $event"
        >
            <DialogContent
                class="max-h-[90vh] overflow-y-auto sm:max-w-[600px]"
            >
                ...
                <div class="space-y-4 py-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <div
                            v-if="availableBillingCycles.length > 0"
                            class="relative"
                        >
                            <select
                                id="billing-cycle"
                        ...
                        <div
                            v-else
                            class="rounded-md border border-input bg-muted px-3 py-2 text-sm text-muted-foreground"
                        >
                            All billing cycles already have prices for this
                            currency.
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            >
                                <option
                                    v-for="status in priceStatusOptions"
                                    :key="status"
                                    :value="status"
                                >
                                    {{
        ...
        <Dialog
            ...
            @update:open="showEditPriceModal = $event"
        >
                ...
                <div class="space-y-4 py-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <Input
                            ...
                            :model-value="
                                editPrice.billingCycle
                                    ? editPrice.billingCycle
                                          .charAt(0)
                                          .toUpperCase() +
                                      editPrice.billingCycle.slice(1)
                                    : ''
                            "
                            readonly
                            class="w-full bg-muted"
        ...
        <Dialog
            ...
            @update:open="showCreateCouponModal = $event"
        >
            <DialogContent
                class="max-h-[90vh] overflow-y-auto sm:max-w-[600px]"
            >
                ...
                <div class="space-y-4 py-4">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div
                            v-if="newCoupon.discountType === 'fixed'"
                            class="space-y-2"
                        >
                            <Label
                                for="coupon-currency"
                    ...
                    <div
                        v-if="newCoupon.durationType === 'recurring'"
                        class="space-y-2"
                    >
                        <Label
                    ...
                    <div class="space-y-2">
                        ...
                        <div
                            v-if="newCoupon.applicablePackages === 'specific'"
                            class="mt-4 rounded-lg border border-border bg-card p-4"
                        >
                            <Label
                                class="mb-3 block text-sm font-medium text-foreground"
                            >
                                Select Packages
                            </Label>
                            <div class="space-y-3">
                                ...
                                >
                                    <input
                                        ...
                                        :id="`package-${pkg.id}`"
                                        :checked="
                                            newCoupon.selectedPackages.includes(
                                                String(pkg.id),
                                            )
                                        "
                                        @change="
                ...
                <DialogFooter>
                    ...
                    <Button
                        ...
                        :disabled="
                            ...
                            !newCoupon.durationType ||
                            !newCoupon.validFrom ||
                            !newCoupon.validUntil ||
                            (newCoupon.applicablePackages === 'specific' &&
                                newCoupon.selectedPackages.length === 0)
                        "
                    >
                        Create Coupon
                    </Button>
        ...
        <!-- Usage History Modal -->
        <Dialog
            :open="showUsageHistoryModal"
            @update:open="showUsageHistoryModal = $event"
        >
            <DialogContent
            ...
            >
                ...
                <div class="space-y-6 py-4">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                        <!-- Total Uses Card -->
                        <div
                            class="rounded-lg border border-purple-200 bg-purple-50 p-4 dark:bg-purple-900/10"
                        >
                            <div
                                class="mb-1 text-sm font-medium text-purple-900 dark:text-purple-300"
                            >
                                TOTAL USES
                            </div>
                            <div
                            ...
                            <div
                                class="mt-1 text-xs text-purple-700 dark:text-purple-400"
                            >
                        ...
                        <!-- Total Discount Card -->
                        <div
                            class="rounded-lg border border-pink-200 bg-gradient-to-br from-pink-50 to-orange-50 p-4 dark:from-pink-900/10 dark:to-orange-900/10"
                        >
                            <div
                                class="mb-1 text-sm font-medium text-pink-900 dark:text-pink-300"
                            >
                                TOTAL DISCOUNT
                            </div>
                            <div
                            ...
                            <div
                                class="mt-1 text-xs text-pink-700 dark:text-pink-400"
                            >
                                Given to customers
                            </div>
                        </div>
                        ...
                        <div
                            class="rounded-lg border border-blue-200 bg-blue-50 p-4 dark:bg-blue-900/10"
                        >
                            <div
                                class="mb-1 text-sm font-medium text-blue-900 dark:text-blue-300"
                            >
                                UNIQUE CUSTOMERS
                            </div>
                            <div
                    ...
                    <!-- Export Tip -->
                    <div
                        class="rounded-lg border border-blue-200 bg-blue-50/50 p-4 dark:bg-blue-900/10"
                    >
                        <div class="flex items-start gap-3">
                            <Lightbulb
                                class="mt-0.5 size-5 flex-shrink-0 text-blue-600"
                            ...
                            >
                                <span class="font-medium">Export Tip:</span> You
                                can copy the entire table data by selecting it
                                and using Ctrl+C (or Cmd+C on Mac) to paste into
        ...
        <Dialog
            ...
            @update:open="showEditCouponModal = $event"
        >
            <DialogContent
                class="max-h-[90vh] overflow-y-auto sm:max-w-[600px]"
            >
                ...
                <div class="space-y-4 py-4">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div
                            v-if="editCoupon.discountType === 'fixed'"
                            class="space-y-2"
                        >
                            <Label
                                for="edit-coupon-currency"
                                class="text-sm font-medium text-foreground"
                            >
                                Currency
                            </Label>
                    ...
                    <!-- Number of Months (shown when Recurring is selected) -->
                    <div
                        v-if="editCoupon.durationType === 'recurring'"
                        class="space-y-2"
                    >
                        <Label
                            for="edit-number-of-months"
                            class="text-sm font-medium text-foreground"
                        >
                            Number of Months
                        </Label>
                    ...
                    <div class="space-y-2">
                        ...
                        <!-- Select Packages (shown when Specific Packages is selected) -->
                        <div
                            v-if="editCoupon.applicablePackages === 'specific'"
                            class="mt-4 rounded-lg border border-border bg-card p-4"
                        >
                            <Label
                                class="mb-3 block text-sm font-medium text-foreground"
                            ...
                            <div class="space-y-3">
                                ...
                                >
                                    <input
                                        ...
                                        :checked="
                                            editCoupon.selectedPackages.includes(
                                                String(pkg.id),
                                            )
                                        "
                                        @change="
                                    ...
                                    <Label
                                        ...
                                        class="cursor-pointer text-sm font-medium text-foreground"
                                        @click.stop="
                                            handleEditLabelClick(pkg.id, $event)
                                        "
                                    >
                                        {{ pkg.name }}
                                    </Label>
                ...
                <DialogFooter>
                    ...
                    <Button
                        ...
                        :disabled="
                            ...
                            !editCoupon.discountValue ||
                            !editCoupon.durationType ||
                            !editCoupon.validFrom ||
                            !editCoupon.validUntil ||
                            (editCoupon.applicablePackages === 'specific' &&
                                editCoupon.selectedPackages.length === 0)
                        "
                    >
                        Update Coupon
                    </Button>
        ...
        <Dialog
            :open="showDeleteFeatureModal"
            @update:open="
                (value) => {
                    if (!value) cancelDeleteFeature();
                }
            "
        >
        ...
        <Dialog
            :open="showDeletePriceModal"
            @update:open="
                (value) => {
                    if (!value) cancelDeletePrice();
                }
            "
        >
            <DialogContent class="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription
                        class="pt-2 text-sm text-muted-foreground"
                    >
                        Are you sure you want to delete this pricing? This
                        action cannot be undone.
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter class="gap-3">
        ...
        <Dialog
            :open="showDeleteCouponModal"
            @update:open="
                (value) => {
                    if (!value) cancelDeleteCoupon();
                }
            "
        >
            <DialogContent class="sm:max-w-md">
